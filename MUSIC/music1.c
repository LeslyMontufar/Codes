...
void beep(int, const int, float, int, int);
...
void beep(int track, const int i, float duracao, int primeira_nota, int n) {
    ...
    int korobeiniki_delay[] = {2, 4, 4, 3, 4, 4, 3, 4, 4, 3,
    ...
    int pumped[] = {// intro
        ...
        // allegro 17
        B[i], A[i], A[i], G[i], A[i], B[i]
        , G[i], A[i],
        B[i], A[i], A[i], A[i], G[i], A[i],
        B[i], A[i], A[i], B[i], G[i], F[i],
        B[i], A[i], A[i], G[i], A[i], G[i], D[i+1], B[i]

        // 22 - 32
        , G[i], A[i],
        B[i], A[i], A[i], A[i], G[i], A[i],
        B[i], A[i], A[i], A[i], A[i], G[i], B[i+1]

        // 33
        , A[i+1], A[i+1], G[i+1], G[i+1], E[i+1], D[i+1],
        G[i+1], G[i+1], G[i+1], E[i+1], D[i+1],
        G[i+1], G[i+1], G[i+1], A[i+1]
    ...
    case 6:
        m.som = pumped;
        m.tempo = pumped_delay;
        m.tamanho = sizeof(pumped)/sizeof(int);
        break;
    }
    ...
    if(sizeof(m.som)!=sizeof(m.tempo)||sizeof(m.som)==0)
        fprintf(stdout, "Os vetores estão com tamanhos diferentes");
    else {
        for(nota=primeira_nota; nota<m.tamanho && !kbhit(); nota++) {
            Beep(*(m.som+nota), duracao* 1000/ *(m.tempo+nota)); // com o 100 deu certo
//            if(track==3)
//                printaPalavra("X", m.som[nota]%(COMPRIMENTO_TELA-6)-3, m.tempo[nota]%(ALTURA_TELA-20)+15, rand()%15+1);

            if(nota == m.tamanho-1 && track==3) {
                if(i<6)
                    beep(track, i+1, duracao, 0, 0);
                else
                    beep(track, 5, duracao, 0, 0);
            }
        }
    }
}

// https://musescore.com/ajplayspiano/scores/4107506
