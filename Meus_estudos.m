clc;
clear all;
close all;

%% Aprendendo Circuitos
if ask('configuracao padrao? (1/0) ')
    f = 60;
    ini_time = 0;
    end_time = 50e-3;
else
    f = input('freq: ');
    ini_time = input('inicial time: ');
    end_time = input('end time: ');
end

T = 1/f;
t = ini_time:end_time/1e3:end_time;
alpha = -cosd(60)+ j*sind(60); %1 fasor de 120
omega = 2*pi*f;
if ask('Delta=1 ou estrela=0? ')
    %% Delta
    fprintf('\t Configuração em delta;\n');
    VL_rms = input('VL_rms: ');
    VAB = VL_rms*sqrt(2)* sin(omega*t);
%     VBC = VAB * alpha^2;
%     VCA = VAB * alpha;
else
    %% Estrela
   disp('Configuracao em estrela') 
end

fprintf('O sistema operando numa frequência de %g, logo com período de %g s\n', f, T);

%% Dados gráficos obtidos
%% No tempo
figure('Name', 'Tensões de linha')
plot(t, VAB);
hold on;
grid on;
% plot(t, VBC);
% plot(t, VCA);
legend('VAB')

%% Fasores
% latex faz isso

%% Funcoes uteis
function bool = ask(question)
    if input(question)==1
        bool = 1;
    else 
        bool = 0;
    end
end