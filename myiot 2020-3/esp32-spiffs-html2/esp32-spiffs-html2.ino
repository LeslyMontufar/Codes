...
// Constants
const char *ssid = "rede"; // Nome da rede, agora o esp vai acessar essa rede
...
void onIndexRequest(AsyncWebServerRequest *request) {
  IPAddress remote_ip = request->client()->remoteIP(); //pagina HTML no IP identificado pelo Async
...
void onPageNotFound(AsyncWebServerRequest *request) {
  ...
  Serial.println("["+remote_ip.toString() +
                  "] HTTP GET request of "+ request->url());
  request->send(404, "text/plain", "Not found"); //404 indica erro e 200 OK requisição bem sucedida
}
...
void setup() {
    ...
    // printa o endereco IP Local do ESP32
    Serial.println();
